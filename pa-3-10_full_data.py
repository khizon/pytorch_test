...
def train(model, data_loader, loss_fxn, optimizer, scheduler, device, start_epoch, epochs):
  ...
  for epoch in range(start_epoch, epochs):
    ...
    if ((epoch+1)%5) == 0:
      ...
      torch.save(checkpoint, f"check-10full-{epoch}.pth.tar")
      print(f'Epoch: {epoch+1} | Loss: {curr_loss} | Checkpoint saved')
    else:
      print(f'Epoch: {epoch+1} | Loss: {curr_loss} |')

    # Save loss and accuracy values
    if not os.path.exists('metrics_10full.csv'):
        with open('metrics_10full.csv', 'w'): pass

    with open(r'metrics_10full.csv', 'a', newline='') as f:
      writer = csv.writer(f)
...
if __name__ == "__main__":
  ...
  if train_continue:
      ...
      if os.path.exists('metrics_10full.csv'):
          os.remove('metrics_10full.csv')
